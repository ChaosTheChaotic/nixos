...
function set_screen_positions()
  ...
  if G.STAGE == G.STAGES.RUN then
    G.seerea.T.x = G.jokers.T.x + G.jokers.T.w + 0.2
    G.seerea.T.y = 2.5
...
function G.FUNCS.draw_from_deck_to_hand(e)
  ...
  G.E_MANAGER:add_event(Event({
    ...
    delay = 0.1,
    func = function()
      -- make sure if we have less than 5 cards in the deck, we only try to copy how many there are
      local card_count = math.min(#G.deck.cards, 5)
