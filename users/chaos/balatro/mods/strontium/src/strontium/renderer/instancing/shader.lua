--- Instanced ubershader builder: per-instance attributes + module composition.
...
local function externs_supported(mod)
    ...
    local cache = mod._externs_cache_instancing
    ...
    for i = 1, #externs do
        ...
        if not ALLOWED_EXTERNS[name] then
            mod._externs_cache_instancing = { body = body, ok = false, bad = name }
            return false, name
        end
    end
    mod._externs_cache_instancing = { body = body, ok = true }
    return true
end
...
function instancing.attach(br)
    function br:invalidate_instanced_shader(layer)
        if not layer then return end
        self.instancing.cache[layer] = nil
        self.instancing.hash[layer] = nil
        self.instancing.errors[layer] = nil
        self.layer_uber_enabled[layer] = nil
    end

    function br:invalidate_all_instanced_shaders()
        local order = self.registry and self.registry.layer_order or nil
        if not order then return end
        for i = 1, #order do
            self:invalidate_instanced_shader(order[i])
        end
    end

    function br:collect_layer_modules_instanced(layer)
        local registry = self.registry
        local version = registry and registry.version or 0

        -- Check module cache
        local cache = self.instancing.layer_modules_cache
        if not cache then
            cache = {}
            self.instancing.layer_modules_cache = cache
        end
        local cached = cache[layer]
            ...
            if mod and module_supports_layer(mod, layer) then
                if not (mod.flags and mod.flags.batch_safe == false) then
                    ...
                    if ok then
                        ...
                        self.instancing.module_errors[key] = "extern_not_allowed:" .. tostring(extern)
                    end
                end
            end
        end
    ...
    function br:build_instanced_ubershader_source(layer, modules)
        ...
        local param_block = [[
    MY_HIGHP_OR_MEDIUMP float param_lo = decode_byte(v_color.b);
    MY_HIGHP_OR_MEDIUMP float param_hi = decode_byte(v_color.a);
...
%s
    ...
    function br:build_instanced_shader(layer)
        ...
        -- Use instanced-specific module collection to ensure proper extern validation
        local modules = self:collect_layer_modules_instanced(layer)
        if #modules == 0 then
            self.layer_uber_enabled[layer] = false
            return nil
        end
        ...
        if cached and self.instancing.hash[layer] == hash then
            return cached
        end

        local source = self:build_instanced_ubershader_source(layer, modules)
        local ok, shader_obj = pcall(love.graphics.newShader, source)
        if not ok then
            print(string.format("[Strontium] Instanced ubershader compile failed for layer '%s': %s", tostring(layer), tostring(shader_obj)))
            self.instancing.cache[layer] = nil
            self.instancing.hash[layer] = hash
            self.instancing.errors[layer] = shader_obj
            self.layer_uber_enabled[layer] = false
            return nil
        end

        self.instancing.cache[layer] = shader_obj
        self.instancing.hash[layer] = hash
        self.instancing.errors[layer] = nil
