    	...
    	".config/rmpc/config.ron".text = ''
    	                      ...
    	                      (
    	                          ...
    	                          status_update_interval_ms: 1000,
    	                              ...
    	                              "--replace-in-metadata", "title,track", "(?i)\\s*(?:[\\[\\(【{<].*?(?:official|video|audio|lyric|visualizer).*?[\\]\\)】}>]|\\s*[-|]\\s*(?:official\\s*video|music\\s*video|official\\s*audio|lyric\\s*video|official\\s*visualizer).*)", "",
    	                              "--no-post-overwrites",
    	                              "--no-playlist",
    	                              "--playlist-items", "1",
																		"--write-subs",
    	                          ...
    	                          tabs: [
    	                      		...
    	                      		'';
    	".config/rmpc/themes/def.ron".text = ''
    	  #![enable(implicit_some)]
    	  #![enable(unwrap_newtypes)]
    	  #![enable(unwrap_variant_newtypes)]
    	  (
